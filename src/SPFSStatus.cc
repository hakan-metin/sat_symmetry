...
void SPFSStatus::generateSPFS(Literal literal, ClauseInjector *injector) {
    std::vector<Literal> literals;
    const BooleanVariable cause = kNoBooleanVariable;
    ...
    DCHECK(!_assignment.literalIsTrue(literals[0]));
    DCHECK(_assignment.literalIsFalse(literals[1]));

    injector->addClause(ClauseInjector::Type::SPFS, cause, std::move(literals));
}
