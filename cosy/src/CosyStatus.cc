...
void CosyStatus::generateESBP(BooleanVariable reason, ClauseInjector *injector) {
...
void CosyStatus::generateForceLexLeaderESBP(BooleanVariable reason,
    ...
    std::vector<Literal> literals;
    std::unordered_set<Literal> used;
    Literal element, inverse, lit, affected, undef;
    ...
    lit = Literal(undef.variable(), _assignment.literalIsTrue(affected));
    if (used.insert(lit).second)
        literals.push_back(lit);

    lit = _assignment.getFalseLiteralForAssignedVariable(affected.variable());
    if (used.insert(lit).second)
        literals.push_back(lit);
