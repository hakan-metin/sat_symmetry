...
bool CNFReader::load(const std::string &filename, CNFModelStats *model_stats) {
    ...
    if (model_stats->numberOfClauses() != expected_num_clauses) {
        LOG(ERROR) << "Expected " << expected_num_clauses <<
            " clauses: found " <<  model_stats->numberOfClauses();
        return false;
    }
    return true;
}
